...
hash` reported NO CHANGE for two documents that refuse and admit the
same input, wherever a schema used an alias as a spread template — the
idiom the alias feature exists for. The hash form is not canon, and it
was rendering an expanded alias through canon, which drops exactly the
`close()` and the marks the hash form exists to add. Anyone pinning such
a schema has a pin that moves on this release, and should re-pin: the
new hash is the one that discriminates.

The other three are a schema that could not mention a checked link, a
termination bound that had never once fired, and a Go-only deadlock
between a mark wrapper and a list inside it. Every fix is in both ports
with rows in the shared suite.
