...
import { computeDiagnostics, computeHover } from '../dist/lsp'
...
describe('coverage2-cli', () => {
  ...
  test('an-interactive-stdout-gives-the-text-figure-its-escapes', () => {
    ...
    try {
      ...
      if (undefined !== no) { process.env.NO_COLOR = no }
      setColor(false)
  ...
  test('the-style-flag-refuses-an-unknown-name-and-escapes-into-a-file', () => {
    const dir = Fs.mkdtempSync(Path.join(Os.tmpdir(), 'aontu-style2-'))
    const file = Path.join(dir, 'm.aon')
    Fs.writeFileSync(file, 'a: 1\n')
    const errs: string[] = []
    const se = process.stderr.write
    ;(process.stderr as any).write = (s: any) => (errs.push(String(s)), true)
    const cap = captureOut()
    // main() sets process.exitCode rather than returning it.
    const code = (argv: string[]): number | undefined => {
      process.exitCode = 0
      cliMain(['node', 'cli', ...argv])
