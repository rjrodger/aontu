...
function escShell(src) {
    return src.split("'").join("'\\''");
}
function unescShell(src) {
    let out = '';
    for (let i = 0; i < src.length; i++) {
        if ("'" !== src[i]) {
            out += src[i];
...
function unescUri(src) {
    const bytes = [];
    for (let i = 0; i < src.length; i++) {
        if ('%' !== src[i]) {
            for (const b of utf8Bytes(src[i])) {
                bytes.push(b);
            }
            continue;
        }
...
function escRegex(src) {
    let out = '';
    for (const ch of src) {
        if (ESC_REGEX_PUNCT.includes(ch)) {
            out += '\\';
        }
        out += ch;
    }
...
function unescRegex(src) {
    ...
    for (let i = 0; i < src.length; i++) {
        ...
        const n = src[i + 1];
        if (undefined === n || !ESC_REGEX_PUNCT.includes(n)) {
            return ['', false];
        }
        out += n;
        i++;
...
function escapeText(src, variant) {
    if ('sq' === variant) {
        return escC(src, "'");
    }
    if ('sql' === variant) {
...
function unescapeText(src, variant) {
    if ('sq' === variant) {
        return unescC(src, "'");
    }
    if ('sql' === variant) {
