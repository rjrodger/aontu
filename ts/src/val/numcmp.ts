...
// A comparison-internal exact rational with a power-of-ten denominator.
// `inf` marks the two binary64 infinities, which some float leaf value
// could in principle carry; NaN cannot reach a scalar (the parser has
// no spelling for it) and is rejected by the atom constructors.
type Scaled = {
  inf?: 1 | -1
...
function scaledOfFloat(f: number): Scaled {
...
function scaledOfNumeric(v: any): Scaled {
  if (v.isBigInteger) {
    return { unscaled: v.peg as bigint, scale: 0 }
  }
  if (v.isBigDecimal) {
    const d = v.peg as Decimal
    return { unscaled: d.unscaled, scale: d.scale }
  }
  // integer and float leaves both hold a binary64.
  return scaledOfFloat(v.peg as number)
}
...
function cmpCodePoints(a: string, b: string): number {
  ...
  while (ai < a.length && bi < b.length) {
    const ac = a.codePointAt(ai) as number
    const bc = b.codePointAt(bi) as number
    if (ac !== bc) {
      return ac < bc ? -1 : 1
    }
    ai += ac > 0xffff ? 2 : 1
...
function scaledIsIntegral(s: Scaled): boolean {
  if (s.inf) {
    return false
  }
  if (0 === s.scale) {
...
function scaledFloor(s: Scaled): bigint {
  if (0 === s.scale) {
    return s.unscaled
  }
  const p = pow10(s.scale)
