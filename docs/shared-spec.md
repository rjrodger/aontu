...
| `mode`   | `canon`, `gen`, `gens`, `err`, `errc`, `errcode`, `vet`, `subsume`, `query`, `why`, `patch`, `diff`, `agentsmd`, `trim`, `hcanon`, `hash`, `fmt`, `fmt-refuse`, `fmt-lint`, `graph`, `relation`, `reaches`, `view`, `views` or `render` (see below) |
