...
| [G9](g9-transformation.md) | Declarative transformation | D | 7 | 1 | 0 | 2 |
...
([ADR-023](../../ADR.md#adr-023--g9-completes-at-the-renderer-the-reflection-sidecar-the-jostraca-bridge-and-string-interpolation-are-retired);
phase 7 came back on 2026-09-11 and that entry is now retired),
...
| **7** — the Jostraca bridge | M | **LANDED 2026-09-11 — the component primitives, not the bridge** | **RETIRED 2026-09-05 by an ADR that is itself now retired; the bridge is still not built and no dependency is taken in either direction.** The write story is `render --out` (all units or nothing, confined below one directory) and `render --check` (the CI form), RENDER.0.md D8; a merge over hand-edited files is a different lifecycle and a downstream tool's, and the two asks on the Jostraca repository (lazy `memfs`, `raw: true`) stay withdrawn. What landed on 2026-09-11 is the other half of the ask, **the component primitives as aontu functions, in both ports**: `project`, `folder`, `file`, `content`, `line`, `fragment`, `slot`, `inject`, `copyfiles` and `listitems`, with `nom` (one name in one of nine spellings — `camel`, `dot`, `kebab`, `pascal`, `path`, `snake`, `text`, `title`, `upper` — or every spelling as a map, with an acronym set that keeps `id` as `ID`) and `translate` (a character map with `a-z` ranges, a short `to` padding with its last character and an omitted `to` deleting). `ts/src/val/CmpFuncVal.ts`, `NomFuncVal.ts` and `TranslateFuncVal.ts` against `go/cmp.go`, `go/nom.go` and `go/translate.go`, function for function. **The FUNCTION is lower case and the NODE it builds carries the Jostraca component name** — `folder("src")` is `{cmp: "Folder", props: {name: "src"}, children: []}`, closed — which is the whole coupling: the plan is data, and whatever consumes it looks the name up. One table in each port states the set (the prop a bare string fills and whether it is required, the bag prop, and the children the component admits), so a child a component does not take is refused at the call rather than at the tree, and a nested list of children splices flat. Declared in `test/spec/signature.tsv` (52 → 64 builtins) and pinned by `test/spec/cmp.tsv` (64 rows, both runners), every expectation taken from the canonical port. Docs: twelve signature sections in [`docs/reference-language.md`](../reference-language.md); both LSP completion lists, both arity tables and the published grammars regenerated. Design: [JOSTRACA.0.md](../design/JOSTRACA.0.md), elevated from SPIKE the same day. **[ADR-023](../../ADR.md#adr-023--g9-completes-at-the-renderer-the-reflection-sidecar-the-jostraca-bridge-and-string-interpolation-are-retired) is RETIRED by this row**, its own enforcement clause reached — *"a future phase that adds a reflection surface, a file-merge dependency or an interpolation syntax supersedes this entry rather than amending a row"* — though what reached the clause is a component set that adds no dependency, and not the bridge that entry refused. **Three discoveries.** (1) **The signature declaration was a prerequisite, not a follow-up.** Go's `derivePositional()` reads `funcSig`, so an undeclared function receives its arguments wrapped in a single list: `nom("planet_body", kebab)` arrived as `nom(["planet_body", "kebab"])` and every component saw one argument. (2) **Two codes refuse the same shape of mistake**, because the signature gate enforces a declared type only where every union word is a scalar kind: `translate(1, "a", "b")` is `func_arg` from the gate, while `folder(1)` — declared `spec: string|map` — reaches the function and is `invalid-arg`. Both are pinned, which is why the file carries two spellings of one mistake. (3) **Arity never reaches a function body.** Every bad count is `func_arity` at parse from the declared signature, so the three functions' own arity guards are unreachable and carry `//coverage:ignore` saying so; what reaches their KIND guards is a top value at an argument (`y: {a: *}` then `folder($.y.a)`), top being no scalar kind and passed through by the gate. **AMENDED 2026-09-12, and the amendment is a REMOVAL.** `render` was taught to lower a component tree to `aontu:code` units, and the work was reverted before it landed. The question that killed it: **the tree is already the representation a component engine consumes** (JOSTRACA.0.md §9 records the pipe as VERIFIED end to end), so no lowering is needed for the components to be a generation road. What the lowering added was measured and found to be a re-spelling, not a capability: the five components it could serve (`project`, `folder`, `file`, `line`, `content`) express nothing `aontu:code` units and `frag` did not already — folder nesting is a path string, `line`/`content` are frag pieces — while `aontu:code` is strictly MORE capable there, a unit carrying declarations and a language lowering where a lowered file is always `text`. The five it refused (`fragment`, `slot`, `inject`, `copyfiles`, `listitems`) are exactly the reason to reach for a component engine, and they cannot be served: each reads or edits a file, and `render-source-has-no-filesystem-access` is a guarded property in both ports ([`docs/trust.md`](../trust.md)), so completing that road needs an ADR and not a patch. The review then found EIGHT defects in the surface, three verified here by hand: a document holding ordinary user data at a `cmp` key was intercepted and its real units discarded; a leaf at the anchor (`copyfiles("assets")`) rendered zero units with verdict `ok`, so `--check` passed vacuously; and the provenance trace produced NO entries while the coverage report named the output tree itself as dead model. Two of those contradicted claims already written into the docs and the CHANGELOG, which is the lesson worth keeping: **the claims were made from a report that ran, not from a report that was read.** What survives is the pair of defects the exercise found. **`line("")` was REFUSED**, so the natural spelling of a blank line errored while `aontu:code` has `%blank` for it: the def now separates "must be present" from "must be non-empty", and a SPAN (`content`, `line`) may be empty where a NAME never may. **The template surface did not compose**: a body line desugars to a bare backtick string, which `%piece` admits and a children list refused, so the writing experience the rb-solar system is built on stopped at the component road; a concrete string child is now `content` sugar, admitted exactly where `content` is. `test/spec/cmp.tsv` +10, and `use-cases/15-code-generation/gen-ts-cmp.aon` is that case's TypeScript generator as a component tree, held by `check.sh` to the bytes the `aontu:code` spelling renders and to the same tree in both ports. The question that exercise left standing — **why `aontu:code` still has a `units` list now that the components exist** — is worked out in [UNITS-AND-TREES.0.md](../design/UNITS-AND-TREES.0.md): a unit's path is module identity that the TypeScript lowering reads and the Go one ignores, a unit's decls are language-neutral where a `content` is bytes for one target, and the four file-touching components sit on the far side of `render-source-has-no-filesystem-access`. That note proposes nothing and opens no phase. |
| **8** — string interpolation | M/L | **RETIRED 2026-09-05 ([ADR-023](../../ADR.md#adr-023--g9-completes-at-the-renderer-the-reflection-sidecar-the-jostraca-bridge-and-string-interpolation-are-retired), itself RETIRED 2026-09-11; this retirement stands on the evidence below and not on that entry)** | Nothing was built. It was deferred behind evidence that `join` did not suffice, and the evidence went the other way: [TEMPLATE.0.md](../design/TEMPLATE.0.md) D3 measured `${expr}` breaking on the project's own material and chose `replace`, whose canonical form has zero concatenations across twelve real handlers. `replace` on a template and `+`/`join` in a body are the two ways a value reaches generated text. |
