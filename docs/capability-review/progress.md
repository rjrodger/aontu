...
| **4** — cross-field arguments, residuation | M | **IN PROGRESS** | Implemented in both ports and byte-identical on 34 of 36 probe sources: an atom whose arguments have not settled becomes `pending` and resolves through `unify`, mirroring `FuncBaseVal`'s discipline (`settle` in `ts/src/val/ConstraintVal.ts`, `go/constraint.go`). All four drafted rows behave as designed, plus `min(1+1)` -> `min(2)`. **Blocked on a TypeScript parser defect** the phase unmasks: in a func-paren comma group TS drops every argument after a `$`-led one (`neq($.z,3)` silently becomes `neq($.z)`) and mis-associates `|` across the comma. Go parses both correctly, so this is an ADR-001 violation in the canonical port. Latent on `main` only because a reference argument was refused there. Rows not yet written. |
| **5** — `must` | S | **IN PROGRESS** | Band B implemented in both ports as a kept, never-simplified field on the residual, folded LAST (`LATE_CJO`, generalised from the sizing rule) so it checks the finished value; error code `must` registered in `errcodes.tsv` with the author message carried into the hint. Same parser blocker as phase 4 — the design's own headline spelling `must("gold"|"silver",msg)` parses in Go and not in TS. Rows not yet written. |
