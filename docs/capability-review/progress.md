...
5. **Counts of the shared suite live here and nowhere else.** All eight
   ...
   register's last update the suite is **110 `.tsv` files, 108
   row-bearing, 4,754 rows**, in twenty-six modes — `errc` 880,
   `canon` 877, `gens` 821, `gen` 601, `err` 291, `view` 174, `errcode`
   157, `fmt` 154, `subsume` 114, `vet` 100, `query` 92, `render` 87,
   `jsonschema` 57, `why` 52, `patch` 41, `hcanon` 40, `views` 37,
   `graph` 37, `fmt-lint` 28, `diff` 28, `relation` 25, `hash` 23,
   ...
   profile rows beside them, and P6 added `gen-form.tsv` and the
   `replace` rows of `gen-emit.tsv`; earlier that day RENDER P2 added the membership rows
...
| [G9](g9-transformation.md) | Declarative transformation | D | 5 | 1 | 1 | 3 |
