...
*Status: design proposal — phases 1–4 (`get` and its projections, and
`why` with its provenance recorder, both ports) are implemented;
phases 5–7 (patch, delivery, REPL) are not. Per-phase status and the
