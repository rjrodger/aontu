...
func TestCompletionsList(t *testing.T) {
	c := Completions()
	// 20 functions + 7 kinds + 4 literals.
	if len(c) != 31 {
		t.Fatalf("expected 31 completions, got %d", len(c))
	}
...
func TestBuiltinFuncNamesParity(t *testing.T) {
	...
	want := []string{"above", "below", "close", "copy", "hide", "key", "length", "lower", "max", "min", "move", "neq", "open", "path", "pref", "re", "super", "type", "unique", "upper"}
	if len(got) != len(want) {
...
func TestHandlerCompletion(t *testing.T) {
	...
	if len(items) != 31 {
		t.Errorf("expected 31 completion items, got %d", len(items))
	}
}
