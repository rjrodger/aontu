...
func TestCompletionsList(t *testing.T) {
	c := Completions()
	if len(c) != 64 {
		t.Fatalf("expected 64 completions, got %d", len(c))
	}
	...
	for _, want := range []string{"close", "upper", "path", "min", "max",
		"above", "below", "neq", "refer", "string", "number",
...
func TestBuiltinFuncNamesParity(t *testing.T) {
	...
	want := []string{"abnf", "above", "acyclic", "add", "below", "close", "copy", "deprecate", "div", "each", "emit", "esc", "filter", "greatest", "hide", "inverse", "join", "key", "least", "length", "list", "lower", "map", "match", "max", "maybe", "min", "mod", "move", "mul", "must", "neq", "open", "pack", "parse", "path", "pick", "pref", "re", "refer", "rel", "rem", "rep", "sort", "split", "sub", "sum", "super", "type", "unique", "upper", "usc"}
	if len(got) != len(want) {
...
func TestHandlerCompletion(t *testing.T) {
	...
	if len(items) != 64 {
		t.Errorf("expected 64 completion items, got %d", len(items))
	}
}
