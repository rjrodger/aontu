...
func TestBuiltinFuncNamesParity(t *testing.T) {
	...
	want := []string{"abnf", "above", "acyclic", "add", "below", "close", "copy", "deprecate", "div", "each", "emit", "esc", "filter", "greatest", "hide", "inverse", "join", "key", "least", "length", "list", "lower", "map", "match", "max", "min", "mod", "move", "mul", "must", "neq", "open", "pack", "parse", "path", "pick", "pref", "re", "refer", "rel", "rem", "rep", "split", "sub", "sum", "super", "type", "unique", "upper", "usc"}
	if len(got) != len(want) {
...
func TestHandlerCompletion(t *testing.T) {
	...
	if len(items) != 62 {
		t.Errorf("expected 62 completion items, got %d", len(items))
	}
}
