...
func TestCompletionsList(t *testing.T) {
	...
	if len(c) != 29 {
		t.Fatalf("expected 29 completions, got %d", len(c))
	}
...
func TestBuiltinFuncNamesParity(t *testing.T) {
	...
	want := []string{"above", "below", "close", "copy", "hide", "key", "lower", "max", "min", "move", "neq", "open", "path", "pref", "re", "super", "type", "upper"}
	if len(got) != len(want) {
...
func TestHandlerCompletion(t *testing.T) {
	...
	if len(items) != 29 {
		t.Errorf("expected 29 completion items, got %d", len(items))
	}
}
