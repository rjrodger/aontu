...
func TestModTidyWritesCanonicalLockfile(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(dir, "aontu_meta", "mod-lock.aon"))
	if nil != err {
		t.Fatal(err)
	}
	lock := string(data)
...
func TestModVendorMaterialisesTheClosure(t *testing.T) {
	...
	modWrite(t, filepath.Join(dir, "aontu_meta", "mod-lock.aon"),
...
// A project that still carries the lockfile or the vendor tree at its
// root, from before they moved under aontu_meta/, is told so once,
// whatever the verb; nothing there is read.
func TestModNamesTheOldLayout(t *testing.T) {
	dir := t.TempDir()
	modWrite(t, filepath.Join(dir, "mod.aon"), "mod: { path: \"corp.example/app\" }\n")
	if err := os.MkdirAll(filepath.Join(dir, "aon_vendor"), 0o755); nil != err {
		t.Fatal(err)
	}
	var out, errw bytes.Buffer
	run([]string{"mod", "verify", dir}, strings.NewReader(""), &out, &errw, false)
	if !strings.Contains(errw.String(), "aon_vendor/ and mod-lock.aon now live under aontu_meta/") {
		t.Fatalf("no hint: %q", errw.String())
	}
	// The lockfile alone at the root is the same hint.
	dir2 := t.TempDir()
	modWrite(t, filepath.Join(dir2, "mod.aon"), "mod: { path: \"corp.example/app\" }\n")
	modWrite(t, filepath.Join(dir2, "mod-lock.aon"), "# mod-lock.aon\n{\"lock\":{}}\n")
	errw.Reset()
	run([]string{"mod", "verify", dir2}, strings.NewReader(""), &out, &errw, false)
	...
	errw.Reset()
	run([]string{"mod", "verify", dir3}, strings.NewReader(""), &out, &errw, false)
	if strings.Contains(errw.String(), "aontu_meta/") {
